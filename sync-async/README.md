Synchronous way: It waits for each operation to complete, after that only it executes the next operation. So, next operation have to wait if current operation is taking time to complete.
